...
**管道状态**：跨模块状态统一落盘 `{baseDir}/output/_state.yaml`，契约见 `{baseDir}/references/pipeline-state.md`。主管道开新一篇文章时重置该文件（保留当天有效的 `flags`）。
...
| 更新 / 更新 WeWrite / 升级 | （就地执行）在 `{baseDir}` 执行 `git pull origin main`，完成后告知版本变化 | |
...
```bash
python3 {baseDir}/scripts/humanness_score.py output/article.md --json
```
...
**第一部分：生成档案**（这篇是怎么来的）
1. 读取 `{baseDir}/history.yaml` 最近一条记录，提取：使用的框架类型 + 写作人格、激活的维度随机化组合、素材采集来源（web_search 还是降级到 LLM）、内容增强策略、范文库是否命中（用了哪几篇 exemplar 还是 fallback 到种子）、playbook 中生效的规则条数。
...
| 用户说 | 动作 |
|--------|------|
| 看看有什么主题 / 主题画廊 | `python3 {baseDir}/toolkit/cli.py gallery`（浏览器内预览全部 16 个主题） |
...
**触发场景**：
1. 主入口 Step 1 发现 `{baseDir}/style.yaml` 不存在（首次使用）
2. 用户说"重新设置风格"、"修改配置"
...
按 onboard.md 的 Phase 1-4 完成：交互式收集信息 → 生成 `{baseDir}/style.yaml` →
...
| 用户说 | 动作 |
|--------|------|
| 学习我的修改 / 我改了，学习一下 | `读取: {baseDir}/references/learn-edits.md`，按其流程执行。支持本地 markdown 修改与微信草稿箱同步（`python3 {baseDir}/scripts/learn_edits.py --from-wechat`） |
| 学习排版 / 学排版 + URL | `python3 {baseDir}/scripts/learn_theme.py <url> --name <name>`，提取后提示用户设置 style.yaml 的 theme 字段 |
| 学习这篇文章 / 导入范文 + URL | `python3 {baseDir}/scripts/fetch_article.py <url> -o /tmp/article.md && python3 {baseDir}/scripts/extract_exemplar.py /tmp/article.md -s <账号名>` |
| 导入范文 + 本地文件 | `python3 {baseDir}/scripts/extract_exemplar.py <文件路径>`（多文件可批量） |
...
1. **源文章**：按优先级解析——用户指定的文件/粘贴内容 > `output/_state.yaml` 的
   ...
   或直接粘贴全文"。确定后把源复制/保存为 `{baseDir}/output/source.md`（质量门要用）。
2. **目标平台**：用户点名了就用；没点名 → 列出 `{baseDir}/toolkit/platforms/` 下的可用
...
1. `读取: {baseDir}/toolkit/platforms/<id>.yaml`，按其 `rewrite_brief`、字数区间、
   标签数、输出格式改写。
