...
"正在发生的事"，爆款是"已被验证的内容需求"——同一角度多个账号都在写、还持续有人写，
说明读者需求真实存在，且不吃时效。
...
- **标题模式**：提炼这批文章的共同标题套路（数字/反差/痛点/身份代入），拟标题时复用

相关度分和切入价值分照常评。**切入价值有一条硬规则**：同题密度高意味着读者已经看过
N 篇同款——必须给出列表里没人写过的差异化角度（对立观点/更深一层/换目标人群），
给不出差异化角度的爆款选题，切入价值封顶 4 分。
