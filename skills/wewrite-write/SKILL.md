...
name: wewrite-write
description: |
  WeWrite 写作模块：给定选题后完成「框架选择 → 素材采集/内容增强 → 反 AI 正文写作」，
  产出 output/article.md。含维度随机化、写作人格、范文注入、自写/写作模型双模式。
  触发关键词：就这个选题写一篇、把这个选题写出来、写公众号正文、按框架写作。
  "写一篇公众号"这类完整需求应触发主入口 wewrite（含选题/配图/发布），不是本模块。
allowed-tools:
  - Bash
  - Read
  - Write
